...
<script type="text/javascript">
		...
		$.ajax({
			...
			success:function(data){
				if(data != null){
					...
					$('#mainframe2').attr("src","${ctx}/reseau/reseauAction!view?id="+nodes[0].id+"&title="+encodeURI(encodeURI(nodes[0].wgmc))+"&menuId=${menuId}");
				}
				else{
					$.messager.alert("警告","加载数据失败");
				}
			}
		});
	}
	...
	function zTreeOnClick(event, treeId, treeNode) {
	   ...
	   var id = treeNode.id;
	   $('#mainframe2').attr("src","${ctx}/reseau/reseauAction!view?id="+id+"&title="+encodeURI(encodeURI(wgmc))+"&menuId=${menuId}");
	}
