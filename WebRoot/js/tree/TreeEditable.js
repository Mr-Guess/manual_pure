 ...
 * 可编辑树，可以进行拖拽的操作
 * 
 * @param loadTreeRemoteURL
 * @param treePanelId
 * @param treeSearchPanelId
 * @param keyInputId
 * @param callback
 * @param onRename
 * @param onRemove
...
Tree = function(loadTreeRemoteURL, treePanelId, treeSearchPanelId, keyInputId,
		callback, onRename, onRemove, iconFromRemote) {
	...
	this.initTree = function(zNodes) {
		...
		var onRenameFunction = function(event, treeId, treeNode) {
			if (typeof t.onRnameFunction == 'function') {
				t.onRname(event, treeId, treeNode);
			}
		};
