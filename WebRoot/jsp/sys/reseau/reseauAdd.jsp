...
$(document).ready(function () {
	$("#title").html(decodeURI('${param.title}') + '配置');
	dataInit();
...
function onAdd() {
	...
	$('#addForm').form('submit', {
		url : '${ctx}/reseau/reseauAction!add',
...
<body style="background-color: white;">
	<center>
		...
		<form id="addForm" method="post" style="width: 900px; height: 450px;">
			<input id="id" type="hidden" name="reseau.id" value="${id}" /> <input
				id="title_s" type="hidden" value="${param.title}" /> <input
				type="hidden" name="reseau.mainTakeId" /> <input type="hidden"
				name="reseau.mainTake" /> <input type="hidden"
				name="reseau.subTakeId" /> <input type="hidden"
				name="reseau.subTake" /> <input type="hidden"
				name="reseau.doubleTakeId" /> <input type="hidden"
				name="reseau.doubleTake" />
			<!--	地区码	-->
			<div style="margin: 0 auto;" align="center" name="addDiv">
				<table class="detailTable" cellpadding="5px"
					...
					<tr>
						...
						<td class="table_con" style="width: 25%;"><textarea
								class="easyui-validatebox" data-options="required:true"
								style="width: 75%; height: 80px;" name="reseau.mainDuty"></textarea></td>
					</tr>
					<tr>
						<td class="table_nemu_new" style="background-color: #f1f6f9">&nbsp;分管负责人:</td>
						<td class="table_con">
							<div id="subName" onclick="otherTakeChose()"
						...
						<td class="table_con"><textarea class="easyui-validatebox"
								data-options="required:true" name="reseau.subDuty"
								style="width: 75%; height: 80px;"></textarea></td>
					</tr>

					<tr>
						<td class="table_nemu_new" style="background-color: #f1f6f9">一岗双责负责人:</td>
						<td class="table_con">
							<div id="doubleName" onclick="doubleTakeChose()"
								style="width: 99%; border: 1px #efefef solid; height: 75px; overflow-y: auto;"></div>
						</td>
						<td class="table_nemu_new" style="background-color: #f1f6f9">&nbsp;安全职责:</td>
						<td class="table_con"><textarea class="easyui-validatebox"
								data-options="required:true" name="reseau.doubleDuty"
								style="width: 75%; height: 80px;"></textarea></td>
					</tr>
					<tr>
						...
						<td class="table_con" colspan="3"><textarea
								...
								data-options="required:true" style="width: 75%; height: 80px;"></textarea>
						</td>
					</tr>
				</table>
				<br>
				<shiro:hasPermission name="${menuId}:add">
					<button type="button" value="" onclick="onAdd()" class="save"></button>
				</shiro:hasPermission>
			</div>
		</form>
	</center>
