...
<script type="text/javascript" src="${ctx}/js/tree/MenuPermissionTreeWithCheckBox.js"></script>
<style>
.top {
     ...
     // 提交添加菜单请求
      ...
      //显示菜单权限树
        function showMenuOptTreeWin(roleId) {
        	$.loding();
        	menuOptTree.cancelAllNodes();
        	$('#menuOptRoleId').val(roleId);
        	$.ajax({
            	url:'${ctx}/sysRole/sysRole!getMenuOptIds?roleId=' + roleId,
                method:'POST',
                success:function(data) {
                	checkByMenuOptIds(data);
                	$.loded();
                	var treeObj = $.fn.zTree.getZTreeObj("menu_opt_tree");
                    var nodes = treeObj.getNodes()[0].children;
                    
                    for(var v in nodes){
                    	if(nodes[v].name == '开发工具'){
                    		treeObj.hideNode(nodes[v]);
                    	}
                    	var nodeChild = nodes[v].children;
                    	for(var c in nodeChild){
                    		if(nodeChild[c].name == ""){
        ...
        function saveMenuOpt(){
        	...
        	var menuNode = $.fn.zTree.getZTreeObj("menu_opt_tree").getCheckedNodes();
        	var menuOpts = [];
      ...
      //显示部门树
        ...
        function setPersons(){
        	...
        	$("#roleUserForm").find("#userIdDiv").remove();
			var userIdDiv = $("<div id='userIdDiv'></div>");
        	var index = 0;
        	for(var i= 0;i<nodes.length;i++){
        		if(nodes[i].nodeType=='User' || nodes[i].nodeType=='Department2'){
        			$(userIdDiv).append("<input type='hidden' name='userIdList["+(index++)+"]' value='"+nodes[i].id+"' />");
        		}
        	}
        	$("#roleUserForm").append($(userIdDiv));
         	submitUsers();
        }
        function submitUsers() {
      /*       
            $('#roleUserForm').form('submit',{
        		url: '${ctx}/sysRole/sysRole!updateRoleUser',
        		success:function(data) {
        			closeAllWin();
                	var data = eval('(' + data + ')');
                    if (data.operateSuccess) {
...
<div class='selectConditionDiv'>
	<form id="searchForm" style="margin: 0;" >
		...
		&nbsp;&nbsp;角色名称:&nbsp;&nbsp;<input class="easyui-validatebox" id="searchRoleName" name="sysRole.roleName" onblur="searchForm();" onkeyup="searchForm();" /> 
		&nbsp;&nbsp;<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-search" onclick="searchForm();">查询</a> &nbsp;
		&nbsp;&nbsp;<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-cancel" onclick="clearSearchForm()">清空</a>
	</form>
</div>
</div>
<div style="margin: auto;">
	 ...
	 <div class="search_add">
	 	<shiro:hasPermission name="54:add">
			<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-add'" onclick="addFunction();">添加</a>
		</shiro:hasPermission>
		<shiro:hasPermission name="54:delete">
			<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-remove'" onclick="removeFunction();">删除</a>
		</shiro:hasPermission>
	</div>
</div>

<div class="data_list_div">
    <table id="data_list" title = "政府权限管理" loadMsg="正在加载,请稍后..." toolbar="#toolbar" fit="true" fitColumns="true">
        <thead>
        <tr>
            <th align="center" width="30%" title="角色名称" field="roleName">角色名称</th>
            <th align="center" width="70%" title="操作" field="id" formatter="gridFormatter">操作</th>         
        </tr>
        </thead>
    </table>
</div>
...
<div id="searchWin" iconCls="icon-save" title="角色信息查询"> 
 ...
 -->
	...
	<form style="height: 0%; weight: 0%" id="roleUserForm">
		...
		<input type="hidden" id="usersStr" name="usersStr" /> 
	</form>
	<!-- 用户组授权窗口 -->
