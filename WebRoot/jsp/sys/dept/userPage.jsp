...
<script type="text/javascript">
        ...
        $(function (){
            ...
            //加载职位
            $(".postion").combobox({
   			 url:'${ctx}/data/dataAction!findByType?parentId=-1&typeId=postion',
   			 valueField:'dataName',
   			 textField:'dataName'
   		 });
        })
     ...
     // 验证密码长度
        ...
        function searchForm() {
          //提交查询表单
			 $('#searchForm').form({
			     url:'${ctx}/user/userAction!pageList.action?usertype=1',
...
//                             '${ctx}/user/userAction!pageList.action?usertype=1',
//                             'data_list', addFunction, editFunction,
//                             viewFunction, removeFunction, searchFunction);
                    grid = new Grid('${ctx}/user/userAction!pageList.action?usertype=1', 'data_list');
                    grid.loadGrid();
        ...
        function gridFormatter(value, rowData, rowIndex) {
        	...
        	+"<input type='button' onclick='editFunction(\""+rowId+"\");'  class='btn1' value='修改'&nbsp;&nbsp;/>";
			</shiro:hasPermission>
        	return url;
        }
        function expFunction(){
        	location.href="${ctx}/user/userAction!exp.action";
        }
...
<!-- 				&nbsp;&nbsp;<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-clear" onclick="clearSearchForm();">清空</a> -->
		...
		<div class="search_add">
			...
			<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-save', plain:true" onclick="expFunction()">导出</a>
	...
	<div style="height: 74%;">
		<table id="data_list" width="100%" style="display: none" toolbar="#toolbar" title="用户管理" fitColumns="true">
			<thead>
				<tr>
	...
	<div style="display: none">
		<div id="addWin" iconCls="icon-save" title="添加用户信息">
			<form style="padding: 10px 20px 10px 20px;" id="addForm"
				method="post">
				<input type="hidden" name="user.userType" value="1" /> <input
					type="hidden" name="user.yhtype" value="gov" /> <input
		...
		<div id="updateWin" iconCls="icon-save" title="更新用户信息">
			<form style="padding: 10px 20px 10px 20px;" id="updateForm"
				...
				<input type="hidden" id="updateId" name="user.id" /> <input
					...
					type="hidden" name="user.userType" value="1" />
				<table cellpadding="5px" style="font-size: 12px;" cellspacing="1"
					border="0" bgcolor="#aed0ea">
					<tr>
						<td class="table_nemu">登录名:</td>
