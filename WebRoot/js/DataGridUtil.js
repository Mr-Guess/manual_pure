...
Grid = function(url, key, setting) {
	...
	this.setting = {
			//title : '',//去除title
			//iconCls : 'icon-edit',//标题图片
			nowrap : false,
			striped : true,
			border : true,
			collapsible : false,
			//fit : true,//适应父容器高度，与fitColumns冲突
			fitColumns:true,//自适应数据区域高度，与fit冲突
			url : this.url,
			remoteSort : false,
			idField : 'id',
			singleSelect : false,
			...
			onLoadSuccess: function(data){
				if (data.total==0) {
					...
					body.find('table tbody').append('<tr><td width="' + body.width() + '" style="height: 50px; text-align: center;"><b>没有数据</b></td></tr>');
				}else{
