	...
	function getFileList(){
		$.post("${ctx}/fileUpload/fileList",{relationId:"${param.relationId}"},
			...
			for(var i in fileList){
				...
				fileDiv.append("<div style='height:30px;align:right;'><a href='javascript:void(0);' onclick='removeClick(\""+file.id+"\",\""+file.fileName+"\")'>删除</a></div>");
...
<body>
	<div class="easyui-layout" data-options="fit:true" doSize="false"
		...
		<div data-options="region:'east',split:true,iconCls:'icon-edit'"
			title="文件上传" style="width: 300px">
			<div style="width: 80%; margin: 50px;">
				<div>
					<form id="form"
						action="${ctx}/fileUpload/upload!uploadFile.action?relationId=${param.relationId}"
						enctype="multipart/form-data" method="post">
						<input id="fileInput" type="file" name="file" /> <input
							type="button" onclick="formClick();" value="提交" /> <input
							id="reset" type="reset" style="display: none;" />
					</form>
					<div>
						<div id="progress" class="easyui-progressbar"
							data-options="value:0"></div>
					</div>
				</div>
			</div>
		</div>
